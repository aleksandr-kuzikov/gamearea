<template>
    <div class="main">
        ...
        <AnimalsGrid v-if="hasAnimals" :animals="animals" :kinds="kinds" @age="ageAnimalRequest" @hoverAnimal="showAnimalInfo" @leaveAnimal="hideAnimalInfo" />
        ...
        methods: {
            ...
            ageAnimalRequest(name) {
                axios
                    ...
                    .then(response => this.animals.map(a => {
                        if (a.name == name) {
                            return {...response.data, icon: catSvg}
                        }
                        return a
                    }))
                    // .then(response => this.$set(this.animals, this.animals.findIndex(a => a.name == name), {...response.data, icon: catSvg}))

                    console.log("age " + name)
            }, 
            showAnimalInfo(name) {
                this.showAnimalInfo = true
                this.hoveredAnimal = name
                console.log(name)
            },
            hideAnimalInfo() {
