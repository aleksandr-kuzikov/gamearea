            ...
            <div @mousewheel="onScroll" ref="scrollArea" class="animals-grid__scroll-area">
                <div ref="grid" class="animals-grid__grid">
                    <Animal v-for="animal in animals" :key="animal.name"
                        :maxSize="kinds.find(k => k.kind == animal.kind_name).max_size" @hoverAnimalR="hoverAnimalF" @leaveAnimalR="leaveAnimalF"
...
export default {
  components: { Animal },
    ...
    methods: {
        ...
        hoverAnimalF(name) {
            this.$emit('hoverAnimalT', name)
        },
        leaveAnimalF() {
            this.$emit('leaveAnimalT')
        }
    },
