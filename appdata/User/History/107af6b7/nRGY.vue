...
export default {
    ...
    methods: {
        ...
        growUp() {
            ...
            this.$refs.animated.style.width = this.animalSize + 'px'
            ...
            if (this.animalSize < this.maxAnimalSize) {
                ...
                this.growUpInterval = setInterval(() => {
                    let cssWidth = Number.parseFloat(this.$refs.animated.style.width)
                    console.log(cssWidth, this.$refs.animated.style.width)
                    if (Math.ceil(cssWidth) >= this.maxAnimalSize) {
                        this.$refs.animated.style.width = this.maxAnimalSize + 'px'
                        clearInterval(this.growUpInterval)
                    }
                    this.$refs.animated.style.width = cssWidth + this.growUpInFrame + 'px'
                }, 100)
            }
        }
    },
